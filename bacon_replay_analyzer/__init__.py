    ...
    def described_fields(self):
        ...
        for idx, value in enumerate(self.fields):
            ...
            if idx in self.KNOWN_FIELDS:
                yield (
                    self.KNOWN_FIELDS[idx],
                    getattr(self, self.KNOWN_FIELDS[idx], self.fields[idx][0]),
                )
...
class Event(BaseEvent):
    KNOWN_FIELDS = {0: "event_type_id", 1: "previous_index"}
    FIRST_DATA_FIELD = 2
...
class Replay:
    ...
    def winner(self):
        ...
        end_events = (
            event
            for event in events
            if event.event_type_id in (EventId.player_wins, EventId.concede)
        )
    ...
    def raw_tuple(self, timestamp):
        ...
        return list(self.read_value(timestamp, index) for index in range(item_count))
    ...
    def raw_tuples(self):
        ...
        section_indexes = sorted(
            (
                ...
                if section not in ("SIZE", "GAME_SETUP")
            ),
            key=int,
        )
    ...
    for idx in sorted(replayEventDict):
        ...
        if eventID == -812:
            ...
            if int_temp(temp, "3_value") == 2:
                ...
                print(styleID, elemDict[styleID])
            else:
                print("FINISHER")
            baseID = int_temp(temp, "4_value")
            print(baseID, elemDict[baseID])
        if eventID == -804:
        ...
        if eventID == -1201:
            ...
            for i in range(anteNum):
                anteID = int(
                    temp[str(i + 4) + "_value"][: temp[str(i + 4) + "_value"].find(".")]
                )
                print(anteID, elemDict[anteID])
        if eventID == -1200:
            print()
